...
export async function loadAllData() {
  ...
  // Fallback to static ./data/ files (generated by sync_service.js)
  try {
    const [fSuc, fLoad, fComp, fMovp, fStock, fCaja] = await Promise.all([
      fetch('./data/db_sucursales.json').then(r => r.ok ? r.json() : null).catch(() => null),
      fetch('./data/db_loaded.json').then(r => r.ok ? r.json() : null).catch(() => null),
      fetch('./data/db_comp.json').then(r => r.ok ? r.json() : null).catch(() => null),
      fetch('./data/db_movp.json').then(r => r.ok ? r.json() : null).catch(() => null),
      fetch('./data/db_stock.json').then(r => r.ok ? r.json() : null).catch(() => null),
      fetch('./data/db_caja.json').then(r => r.ok ? r.json() : null).catch(() => null)
    ]);

    if (fSuc || fComp || fMovp || fStock || fCaja) {
      if (fSuc && fSuc.length) setSucursales(fSuc);
      if (fLoad && fLoad.length) setLoaded(fLoad);
      ...
      if (fComp) fComp.forEach(r => { r.anio=pN(r.anio); r.mes=pN(r.mes); r.dia=pN(r.dia); r.dow=pN(r.dow); r.importe=pI(r.importe); addDBRecord('comp', mkK('comp', r), r); });
      if (fMovp) fMovp.forEach(r => { r.anio=pN(r.anio); r.mes=pN(r.mes); r.dia=pN(r.dia); r.dow=r.dow!=null?pN(r.dow):null; r.salida=pI(r.salida); r.entrada=pI(r.entrada); r.importe=pI(r.importe); addDBRecord('movp', mkK('movp', r), r); });
      if (fStock) fStock.forEach(r => { r.stock=pI(r.stock); r.unidades=pI(r.unidades); r.imp_costo=pI(r.imp_costo); r.imp_venta=pI(r.imp_venta); addDBRecord('stock', mkK('stock', r), r); });
      if (fCaja) fCaja.forEach(r => { r.anio=pN(r.anio); r.mes=pN(r.mes); r.dia=pN(r.dia); r.ventas=pI(r.ventas); r.gastos=pI(r.gastos); r.tarjetas=pI(r.tarjetas); r.efectivo=pI(r.efectivo); addDBRecord('caja', mkK('caja', r), r); });

      console.log('[KBI] Datos cargados de archivos de datos locales.');
      updateCloudUI('ok');
      return true;
    }
  } catch (err) {
    console.log('[KBI] No static data files found, trying IndexedDB...');
  }
